...
export default async function DashboardPage() {
  ...
  try {
    ...
    if (result.workspaces.length > 0) {
      ...
      workspace = await callMCPServer<Workspace>("workspace_create", {
        name: "My Workspace",
        slug: "my-workspace",
    ...
    if (workspace) {
      const [orgsRes, campaignsRes, tasksRes, evidenceRes, trialsRes, biomarkersRes] =
        await Promise.allSettled([
          callMCPServer<ListResponse>("advocacy_org_list", { workspaceId: workspace.id }),
          callMCPServer<ListResponse>("campaign_list", { workspaceId: workspace.id }),
          callMCPServer<ListResponse>("task_list", { workspaceId: workspace.id }),
          callMCPServer<ListResponse>("evidence_list", {}),
          callMCPServer<ListResponse>("clinical_trial_list", {}),
          callMCPServer<ListResponse>("biomarker_list", {}),
        ]);
  ...
  const slug = workspace?.slug ?? "caid-demo";
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Use Archon to add new features to CAID. Describe what you want and Archon will plan,
              implement, and PR it.
            </p>
            <Link
              href={`/${slug}/archon`}
